...
### Fresh Ubuntu Server Setup (Common for Cloud Instances)

If starting on a fresh Ubuntu server (e.g., AWS, DigitalOcean), system packages must be installed before Manim will build:
...
sudo apt-get install -y python3-pip python3-venv \
  ...
  libcairo2-dev pkg-config cmake libpango1.0-dev libpangocairo-1.0-0 \
  ffmpeg

# 2. Create virtual environment (externally-managed Python requires this)
python3 -m venv ~/manim-env
source ~/manim-env/bin/activate

# 3. Install Manim inside venv
pip install manim
...
- `pangocairo >= 1.30.0 is required` → install `libpango1.0-dev libpangocairo-1.0-0`
- `cairo` not found → install `libcairo2-dev pkg-config`
- `externally-managed-environment` → use `python3 -m venv` instead of system pip
- `cmake` not found → install `cmake`

**Always use the venv binary:** `~/manim-env/bin/manim` or activate the venv first.
...
| **Sales pitch video** | Product/service offering | Problem-solution arc with pricing and CTA | `references/sales-pitch-video.md` |
...
| File | Contents |
|------|----------|
| `references/sales-pitch-video.md` | Sales pitch videos with Manim — problem-solution arc, brand colors, pricing cards, CTA, file delivery |
| `references/audio-sync.md` | Post-production audio sync — tpad/apad filters, duration matching, ffmpeg muxing |
