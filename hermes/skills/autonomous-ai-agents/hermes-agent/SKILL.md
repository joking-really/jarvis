...
- **WhatsApp: no markdown rendering**: WhatsApp messages render as plain text. Do not use markdown formatting (no `**bold**`, no `\`code\``, no `[links](url)`). Use plain text only. To send media (images, videos, files), include `MEDIA:/absolute/path/to/file` in your response — the gateway delivers it as a native attachment. Images appear as photos, videos play inline, other files arrive as downloadable documents.
